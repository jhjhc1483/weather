...
프로젝트에서 사용하는 5개 주요 공공데이터 API 전체의
응답 속도, 결과 코드, JSON 정상 반환 유무, 장애 발생 여부를 종합적으로 진단합니다.
...
# 주요 테스트 대상
TEST_LOCATIONS = [
    {'name': '양평', 'nx': 65, 'ny': 123, 'station': '양평읍'},
    {'name': '세종', 'nx': 66, 'ny': 103, 'station': '아름동'},
    {'name': '계룡', 'nx': 65, 'ny': 99, 'station': '엄사면'},
]
...
def main():
    ...
    print(f" 서비스키 : 길이 {len(key)}자 (...{key[-4:]})")
    print("=" * 72)

    results = []

    # 1. 기상특보 API
    print("\n[1/5] 기상청 기상특보 API (getWthrWrnMsg)")
    ...
    # 2. 초단기실황 API
    print("\n[2/5] 기상청 초단기실황 API (getUltraSrtNcst)")
    ...
    # 3. 초단기예보 API
    print("\n[3/5] 기상청 초단기예보 API (getUltraSrtFcst)")
    ...
    # 4. 단기예보 API
    print("\n[4/5] 기상청 단기예보 API (getVilageFcst)")
    for loc in TEST_LOCATIONS:
        ok, elapsed, msg, size = test_api_call(
    ...
    # 5. 에어코리아 미세먼지 API
    print("\n[5/5] 에어코리아 미세먼지 API (getMsrstnAcctoRltmMesureDnsty)")
    for loc in TEST_LOCATIONS:
        ok, elapsed, msg, size = test_api_call(
